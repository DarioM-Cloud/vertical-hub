...
import { Route, CheckCircle2, Filter } from 'lucide-react';
...
export default function ListadoVias({ vias, onOpenModal }) {
  ...
  const viasFiltradas = vias.filter(via => {
    ...
    return coincideGrado && coincideColor && coincideSector;
  });
      ...
      <div className={styles.viasGrid}>
        {viasFiltradas.length === 0 ? (
          ...
          viasFiltradas.map(via => (
            <div key={via.id} className={styles.viaCard}>
              ...
              <Button variant="outline" className={styles.viaBtn} onClick={() => onOpenModal(via)}>
